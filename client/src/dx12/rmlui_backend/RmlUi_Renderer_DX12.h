...
private:
	...
	static constexpr size_t NumPrograms = 26;
...
public:
	...
	public:
		...
		void Set_External(bool external) noexcept { m_is_external = external; }
		bool Is_External() const noexcept { return m_is_external; }
		void Set_NV12(bool nv12) noexcept { m_is_nv12 = nv12; }
		bool Is_NV12() const noexcept { return m_is_nv12; }
		void Set_AuxiliaryResource(ID3D12Resource* resource) noexcept { m_p_auxiliary_resource = resource; }
		...
		void Destroy()
		{
			...
			if (m_p_resource)
			{
				if (m_p_auxiliary_resource)
				{
					m_p_auxiliary_resource->Release();
					m_p_auxiliary_resource = nullptr;
				}
	...
	private:
		...
		ID3D12Resource* m_p_auxiliary_resource = nullptr;
		bool m_is_external = false;
		bool m_is_nv12 = false;
...
public:
	...
	bool Is_StencilEnabled() const { return m_is_stencil_enabled; }
	UINT Get_StencilReference() const { return m_stencil_ref_value; }
	TextureMemoryManager& Get_TextureManager();
	BufferMemoryManager& Get_BufferManager();

	// Registers a caller-owned committed texture in the backend's shader-visible
	// descriptor heap. The returned handle follows normal ReleaseTexture rules.
	Rml::TextureHandle RegisterExternalTexture(ID3D12Resource* resource, DXGI_FORMAT format);
	// Registers either a packed DXGI_FORMAT_NV12 texture (chroma == nullptr) or
	// separate R8 luma and R8G8 chroma textures for direct planar sampling.
	Rml::TextureHandle RegisterExternalNV12Texture(ID3D12Resource* luma, ID3D12Resource* chroma = nullptr);

	// Waits until all work submitted by this renderer has completed.
	void WaitIdle();
...
private:
	...
	void Create_Resource_Pipeline_TextureVariant(ProgramId stencil_always, ProgramId stencil_equal,

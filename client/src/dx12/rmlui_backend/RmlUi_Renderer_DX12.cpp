...
// Direct NV12 presentation. The texture handle binds t0 to the full-resolution
// luma plane and t1 to the half-resolution interleaved chroma plane. Keeping
// the color conversion in this final RmlUi draw avoids an intermediate RGBA
// texture and a separate compute/CUDA dispatch.
static constexpr const char pShaderSourceText_Texture_NV12[] = R"(
struct VS_INPUT
{
	float4 pos : SV_Position;
	float4 color : COLOR;
...
float4 main(const VS_INPUT IN) : SV_TARGET
{
	const float y = max(0.0, g_Luma.Sample(g_SamplerLinear, IN.uv) - (16.0 / 255.0));
	const float2 uv = g_Chroma.Sample(g_SamplerLinear, IN.uv) - (128.0 / 255.0);
	const float c = (298.0 / 256.0) * y;
	const float3 rgb = float3(
		c + (409.0 / 256.0) * uv.y,
		c - (100.0 / 256.0) * uv.x - (208.0 / 256.0) * uv.y,
		c + (516.0 / 256.0) * uv.x);
	return IN.color * float4(saturate(rgb), 1.0);
}
)";
...
enum class ProgramId : int {
	...
	Texture_NV12_Stencil_Always,
	Texture_NV12_Stencil_Equal,
	Texture_NV12_Stencil_Disabled,
	Gradient,
	Creation,
	...
	m_fence_screenshot_value{}, m_precompiled_fullscreen_quad_geometry{}
{
	...
	// Framebuffers own one SRV per RTV/DSV slot in the tail of the shader heap.
	// Video frame replacement and UI uploads must not consume those descriptors:
	// the postprocess framebuffer may be created only in EndFrame after a resize.
	static_assert(RMLUI_RENDER_BACKEND_FIELD_DESCRIPTORAMOUNT_FOR_SRV_CBV_UAV >
		RMLUI_RENDER_BACKEND_FIELD_DESCRIPTOR_HEAP_RTV + RMLUI_RENDER_BACKEND_FIELD_DESCRIPTOR_HEAP_DSV);
	m_p_offset_allocator_for_descriptor_heap_shaders =
		new OffsetAllocator::Allocator((RMLUI_RENDER_BACKEND_FIELD_DESCRIPTORAMOUNT_FOR_SRV_CBV_UAV -
			RMLUI_RENDER_BACKEND_FIELD_DESCRIPTOR_HEAP_RTV - RMLUI_RENDER_BACKEND_FIELD_DESCRIPTOR_HEAP_DSV) * m_size_descriptor_heap_shaders);
...
RenderInterface_DX12::~RenderInterface_DX12()
{
...

	{
		...
		if (m_p_device)
		{
			// D3D12CreateDevice can return the same COM device to multiple renderer
			// instances on the same adapter. Resources owned by another HWND then
			// legitimately keep the shared device alive after this renderer exits.
			m_p_device->Release();
		}
	}
...
bool RenderInterface_DX12::BeginFrame()
{
	...
	if (m_p_command_graphics_list)
	{
		...
		if (!m_manager_render_layer.GetTopLayer().Get_Texture())
		{
			m_manager_render_layer.EndFrame();
			RMLUI_DX_MARKER_END(m_p_command_graphics_list);
			m_p_command_graphics_list->Close();
			return false;
		}
...
void RenderInterface_DX12::EndFrame()
{
	...
	if (m_p_command_graphics_list)
	{
		...
		if (fb_active.Get_Texture() && fb_postprocess.Get_Texture())
		{
			...
			// The window setting can disable MSAA even when the compiled default enables it.
			if (m_msaa_sample_count > 1)
			{
...
{
	if (!m_manager_render_layer.GetTopLayer().Get_Texture()) return;
	...
	else if (texture)
	{
		...
		const bool is_nv12 = texture != TextureEnableWithoutBinding && p_handle_texture->Is_NV12();

		if (m_is_stencil_enabled)
		{
			if (m_is_stencil_equal)
			{
				UseProgram(is_nv12 ? ProgramId::Texture_NV12_Stencil_Equal : ProgramId::Texture_Stencil_Equal);
			}
			else
			{
				UseProgram(is_nv12 ? ProgramId::Texture_NV12_Stencil_Always : ProgramId::Texture_Stencil_Always);
			}
		}
		else
		{
			UseProgram(is_nv12 ? ProgramId::Texture_NV12_Stencil_Disabled : ProgramId::Texture_Stencil_Disabled);
		}
...
Rml::LayerHandle RenderInterface_DX12::RenderLayerStack::PushLayer()
{
	...
	if (m_layers_size == static_cast<int>(m_fb_layers.size()) || !m_fb_layers[m_layers_size].Get_Texture())
	{
		...
		auto* p_buffer = &m_fb_layers[m_layers_size];
		if (m_p_depth_stencil_for_layers->Get_Texture())
			CreateFramebuffer(p_buffer, m_width, m_height, m_msaa_sample_count, false);
		p_buffer->Set_ID(m_layers_size);

#ifdef RMLUI_DX_DEBUG
		if (p_buffer->Get_Texture())
		{
	...
	bool is_depth_stencil)
{
	...
	if (m_p_manager_texture)
	{
		...
		auto* allocated = m_p_manager_texture->Alloc_Texture(desc_texture, p_result, flags, states
#ifdef RMLUI_DX_DEBUG
			,
...
Rml::LayerHandle RenderInterface_DX12::PushLayer()
{
	...
	if (!framebuffer.Get_Texture()) return layer_handle;
...
{
	if (!framebuffer.Get_Texture()) return;
	RMLUI_ASSERTMSG(m_p_command_graphics_list, "early calling must be initialized before calling this method!");
...
void RenderInterface_DX12::RenderFilters(Rml::Span<const Rml::CompiledFilterHandle> filter_handles)
{
	...
	if (filter_handles.empty()) return;
	if (!m_manager_render_layer.GetPostprocessPrimary().Get_Texture() ||
		!m_manager_render_layer.GetPostprocessSecondary().Get_Texture()) return;
	for (const auto handle : filter_handles)
	{
		const auto type = reinterpret_cast<const CompiledFilter*>(handle)->type;
		if (type == FilterType::DropShadow && !m_manager_render_layer.GetPostprocessTertiary().Get_Texture()) return;
		if (type == FilterType::MaskImage && !m_manager_render_layer.GetBlendMask().Get_Texture()) return;
	}
	...
	Rml::Span<const Rml::CompiledFilterHandle> filters)
{
	...
	if (!m_manager_render_layer.GetLayer(source).Get_Texture() ||
		!m_manager_render_layer.GetLayer(destination).Get_Texture() ||
		!m_manager_render_layer.GetPostprocessPrimary().Get_Texture()) return;
...
Rml::TextureHandle RenderInterface_DX12::SaveLayerAsTexture()
{
	...
	if (!m_manager_render_layer.GetTopLayer().Get_Texture() ||
		!m_manager_render_layer.GetPostprocessPrimary().Get_Texture() ||
		!m_manager_render_layer.GetPostprocessSecondary().Get_Texture()) return {};
...
Rml::CompiledFilterHandle RenderInterface_DX12::SaveLayerAsMaskImage()
{
	...
	if (!m_manager_render_layer.GetTopLayer().Get_Texture() ||
		!m_manager_render_layer.GetPostprocessPrimary().Get_Texture() ||
		!m_manager_render_layer.GetBlendMask().Get_Texture()) return {};
...
ID3D12GraphicsCommandList* RenderInterface_DX12::Get_CommandList() const
{
	return m_p_command_graphics_list;
}

Rml::TextureHandle RenderInterface_DX12::RegisterExternalTexture(ID3D12Resource* resource, DXGI_FORMAT format)
{
	if (!resource || !m_p_device || !m_p_offset_allocator_for_descriptor_heap_shaders)
		return {};

	auto* texture = new TextureHandleType();
	resource->AddRef();
	texture->Set_Resource(resource);
	texture->Set_External(true);

	auto descriptor_allocation = m_p_offset_allocator_for_descriptor_heap_shaders->allocate(
		static_cast<OffsetAllocator::uint32>(m_size_descriptor_heap_shaders));
	...
	D3D12_CPU_DESCRIPTOR_HANDLE destination = m_handle_shaders;
	destination.ptr += descriptor_allocation.offset;
	m_p_device->CreateShaderResourceView(resource, &srv, destination);
	texture->Set_Allocation_DescriptorHeap(descriptor_allocation);
	return reinterpret_cast<Rml::TextureHandle>(texture);
}

Rml::TextureHandle RenderInterface_DX12::RegisterExternalNV12Texture(ID3D12Resource* luma, ID3D12Resource* chroma)
{
	if (!luma || !m_p_device || !m_p_offset_allocator_for_descriptor_heap_shaders)
		return {};

	const D3D12_RESOURCE_DESC luma_desc = luma->GetDesc();
	if (chroma)
	{
		const D3D12_RESOURCE_DESC chroma_desc = chroma->GetDesc();
		if ((luma_desc.Format != DXGI_FORMAT_R8_UNORM && luma_desc.Format != DXGI_FORMAT_NV12) ||
			chroma_desc.Format != DXGI_FORMAT_R8G8_UNORM)
			return {};
	}
	else if (luma_desc.Format != DXGI_FORMAT_NV12)
	{
		return {};
	}
	...
	auto* texture = new TextureHandleType();
	luma->AddRef();
	texture->Set_Resource(luma);
	texture->Set_External(true);
	texture->Set_NV12(true);
	if (chroma)
	{
		chroma->AddRef();
		texture->Set_AuxiliaryResource(chroma);
	}

	auto descriptor_allocation = m_p_offset_allocator_for_descriptor_heap_shaders->allocate(
		static_cast<OffsetAllocator::uint32>(2 * m_size_descriptor_heap_shaders));
	if (descriptor_allocation.offset == OffsetAllocator::Allocation::NO_SPACE)
	{
		texture->Destroy();
		delete texture;
	...
	m_p_device->CreateShaderResourceView(luma, &srv, destination);

	destination.ptr += m_size_descriptor_heap_shaders;
	srv.Format = DXGI_FORMAT_R8G8_UNORM;
	srv.Texture2D.PlaneSlice = chroma ? 0 : 1;
	m_p_device->CreateShaderResourceView(chroma ? chroma : luma, &srv, destination);

	texture->Set_Allocation_DescriptorHeap(descriptor_allocation);
	return reinterpret_cast<Rml::TextureHandle>(texture);
}

void RenderInterface_DX12::WaitIdle()
{
	Flush();
}
...
void RenderInterface_DX12::Create_Resource_Pipeline_Texture()
{
	...
	Create_Resource_Pipeline_TextureVariant(
		ProgramId::Texture_Stencil_Always, ProgramId::Texture_Stencil_Equal,
		ProgramId::Texture_Stencil_Disabled, 1, pShaderSourceText_Texture, false, L"Texture");
	Create_Resource_Pipeline_TextureVariant(
		ProgramId::Texture_NV12_Stencil_Always, ProgramId::Texture_NV12_Stencil_Equal,
		ProgramId::Texture_NV12_Stencil_Disabled, 2, pShaderSourceText_Texture_NV12, true, L"Texture_NV12");
}

void RenderInterface_DX12::Create_Resource_Pipeline_TextureVariant(ProgramId stencil_always, ProgramId stencil_equal,
	ProgramId stencil_disabled, uint32_t num_descriptors, const char* pixel_shader_source,
	bool clamp_sampler, const wchar_t* debug_name)
{
	RMLUI_ZoneScopedN("DirectX 12 - Create_Resource_Pipeline_TextureVariant");
	RMLUI_ASSERTMSG(m_p_device, "must be valid when we call this method!");
	...
	if (m_p_device)
	{
		...
		ranges[0].NumDescriptors = num_descriptors;
		...
		sampler.AddressU = clamp_sampler ? D3D12_TEXTURE_ADDRESS_MODE_CLAMP : D3D12_TEXTURE_ADDRESS_MODE_WRAP;
		sampler.AddressV = clamp_sampler ? D3D12_TEXTURE_ADDRESS_MODE_CLAMP : D3D12_TEXTURE_ADDRESS_MODE_WRAP;
		sampler.AddressW = clamp_sampler ? D3D12_TEXTURE_ADDRESS_MODE_CLAMP : D3D12_TEXTURE_ADDRESS_MODE_WRAP;
		...
		status = m_p_device->CreateRootSignature(0, p_signature->GetBufferPointer(), p_signature->GetBufferSize(),
			IID_PPV_ARGS(&m_root_signatures[static_cast<int>(stencil_always)]));
		RMLUI_DX_VERIFY_MSG(status, "failed to CreateRootSignature");

		status = m_p_device->CreateRootSignature(0, p_signature->GetBufferPointer(), p_signature->GetBufferSize(),
			IID_PPV_ARGS(&m_root_signatures[static_cast<int>(stencil_equal)]));
		RMLUI_DX_VERIFY_MSG(status, "failed to CreateRootSignature");

		status = m_p_device->CreateRootSignature(0, p_signature->GetBufferPointer(), p_signature->GetBufferSize(),
			IID_PPV_ARGS(&m_root_signatures[static_cast<int>(stencil_disabled)]));
		RMLUI_DX_VERIFY_MSG(status, "failed to CreateRootSignature");
		...
		status = D3DCompile(pixel_shader_source, std::strlen(pixel_shader_source), nullptr, nullptr, nullptr, "main", "ps_5_0",
			m_default_shader_flags, 0, &p_shader_pixel, &p_error_buff);
#ifdef RMLUI_DX_DEBUG
		if (FAILED(status))
		{
		...
		desc_pipeline.InputLayout = desc_input_layout;
		desc_pipeline.pRootSignature = m_root_signatures[static_cast<int>(stencil_always)];
		desc_pipeline.VS = desc_bytecode_vertex_shader;
		desc_pipeline.PS = desc_bytecode_pixel_shader;
		desc_pipeline.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
		desc_pipeline.RTVFormats[0] = RMLUI_RENDER_BACKEND_FIELD_COLOR_TEXTURE_FORMAT;
		...
		status =
			m_p_device->CreateGraphicsPipelineState(&desc_pipeline, IID_PPV_ARGS(&m_pipelines[static_cast<int>(stencil_always)]));
		...
		desc_pipeline.DepthStencilState = desc_depth_stencil;
		desc_pipeline.pRootSignature = m_root_signatures[static_cast<int>(stencil_equal)];

		status =
			m_p_device->CreateGraphicsPipelineState(&desc_pipeline, IID_PPV_ARGS(&m_pipelines[static_cast<int>(stencil_equal)]));
		...
		desc_pipeline.DepthStencilState = desc_depth_stencil;
		desc_pipeline.pRootSignature = m_root_signatures[static_cast<int>(stencil_disabled)];

		status = m_p_device->CreateGraphicsPipelineState(&desc_pipeline,
			IID_PPV_ARGS(&m_pipelines[static_cast<int>(stencil_disabled)]));
		RMLUI_DX_VERIFY_MSG(status, "failed to CreateGraphicsPipelineState (Texture_Stencil_Disabled)");

#ifdef RMLUI_DX_DEBUG
		m_root_signatures[static_cast<int>(stencil_always)]->SetName(debug_name);
		m_pipelines[static_cast<int>(stencil_always)]->SetName(debug_name);
		m_root_signatures[static_cast<int>(stencil_equal)]->SetName(debug_name);
		m_pipelines[static_cast<int>(stencil_equal)]->SetName(debug_name);
		m_root_signatures[static_cast<int>(stencil_disabled)]->SetName(debug_name);
		m_pipelines[static_cast<int>(stencil_disabled)]->SetName(debug_name);
#endif
	}
}
...
constexpr const char* translate_programId(ProgramId id)
{
	...
	case ProgramId::Texture_NV12_Stencil_Always: return "Texture_NV12_Stencil_Always";
	case ProgramId::Texture_NV12_Stencil_Equal: return "Texture_NV12_Stencil_Equal";
	case ProgramId::Texture_NV12_Stencil_Disabled: return "Texture_NV12_Stencil_Disabled";
...
)
{
	...
	const auto descriptor = m_p_offset_allocator_for_descriptor_heap_srv_cbv_uav->allocate(
		static_cast<OffsetAllocator::uint32>(m_size_srv_cbv_uav_descriptor));
	if (descriptor.offset == OffsetAllocator::Allocation::NO_SPACE)
		return nullptr;
	p_impl->Set_Allocation_DescriptorHeap(descriptor);
	...
	if (!p_result)
	{
		m_p_offset_allocator_for_descriptor_heap_srv_cbv_uav->free(descriptor);
		p_impl->Set_Allocation_DescriptorHeap({});
	}
	return p_result;
}
...
)
{
	...
	if (!Alloc_As_Committed(base_memory_size_for_allocation_in_bytes, total_memory_for_allocation, desc, initial_state,
		p_impl->Get_Texture(), p_impl))
		return nullptr;
...
void RenderInterface_DX12::TextureMemoryManager::Free_Texture(TextureHandleType* p_texture)
{
	...
	if (p_texture)
	{
		...
		if (!p_texture->Is_External() && index != -1)
		{
			m_blocks.at(index)->FreeAllocation(p_texture->Get_Info().alloc_info);
		}

		// Framebuffer SRVs share the lifetime of their RTV/DSV slots. Only
		// uploaded and external textures have shader-heap allocator metadata.
		if (m_p_offset_allocator_for_descriptor_heap_srv_cbv_uav &&
			p_texture->Get_Allocation_DescriptorHeap().metadata != OffsetAllocator::Allocation::NO_SPACE)
		{
	...
	Gfx::FramebufferData* p_impl)
{
	...
	if (m_p_allocator)
	{
		...
		if (FAILED(status) || !p_resource || !p_allocation)
		{
			if (p_resource) p_resource->Release();
			if (p_allocation) p_allocation->Release();
			return false;
		}
		...
		const auto view = is_rt
			? Alloc_RenderTargetResourceView(p_resource, p_impl->Get_VirtualAllocation_Descriptor())
			: Alloc_DepthStencilResourceView(p_resource, p_impl->Get_VirtualAllocation_Descriptor());
		if (!view.ptr)
		{
			p_resource->Release();
			p_allocation->Release();
		...
		p_impl->Set_DescriptorResourceView(view);

		// Each RTV/DSV slot has a matching SRV in the reserved heap tail. This
		// remains available even when every ordinary texture descriptor is held.
		auto* const view_heap = is_rt ? m_p_descriptor_heap_rtv : m_p_descriptor_heap_dsv;
		const size_t view_stride = is_rt ? m_size_rtv_descriptor : m_size_dsv_descriptor;
		const size_t view_index = (view.ptr - view_heap->GetCPUDescriptorHandleForHeapStart().ptr) / view_stride;
		const size_t reserved_begin = RMLUI_RENDER_BACKEND_FIELD_DESCRIPTORAMOUNT_FOR_SRV_CBV_UAV -
			RMLUI_RENDER_BACKEND_FIELD_DESCRIPTOR_HEAP_RTV - RMLUI_RENDER_BACKEND_FIELD_DESCRIPTOR_HEAP_DSV;
		const size_t srv_index = reserved_begin + view_index + (is_rt ? 0 : RMLUI_RENDER_BACKEND_FIELD_DESCRIPTOR_HEAP_RTV);
		OffsetAllocator::Allocation descriptor_allocation{};
		descriptor_allocation.offset = static_cast<OffsetAllocator::uint32>(srv_index * m_size_srv_cbv_uav_descriptor);
		const D3D12_CPU_DESCRIPTOR_HANDLE destination{m_p_handle->ptr + descriptor_allocation.offset};
		m_p_device->CreateShaderResourceView(p_resource, &desc_srv, destination);
	...
	const Rml::byte* p_data, ID3D12Resource* p_resource)
{
	...
	const auto descriptor_allocation = p_texture_handle->Get_Allocation_DescriptorHeap();
		...
		if (m_p_virtual_block_for_depth_stencil_heap_allocations)
		{
			if (m_p_device)
			{
				if (m_p_descriptor_heap_dsv)
				{
					...
					if (FAILED(status))
					{
						Rml::Log::Message(Rml::Log::LT_ERROR, "[DirectX 12] Framebuffer view descriptor heap exhausted");
						return {};
					}
					...
					desc_rtv.ViewDimension = p_resource->GetDesc().SampleDesc.Count > 1 ? D3D12_DSV_DIMENSION_TEXTURE2DMS : D3D12_DSV_DIMENSION_TEXTURE2D;
		...
		if (m_p_virtual_block_for_render_target_heap_allocations)
		{
			if (m_p_device)
			{
				if (m_p_descriptor_heap_rtv)
				{
					...
					desc_rtv.ViewDimension = p_resource->GetDesc().SampleDesc.Count > 1 ? D3D12_RTV_DIMENSION_TEXTURE2DMS : D3D12_RTV_DIMENSION_TEXTURE2D;

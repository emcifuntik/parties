...
RenderInterface_VK::RenderInterface_VK(void* native_window, const Backend::RmlRendererSettings& settings) :
	...
	m_swapchain_format{}, m_texture_depthstencil{}, m_pending_for_deletion_textures_by_frames{}
{
	...
	Rml::Vector<const char*> required_extensions;
	required_extensions.push_back(VK_KHR_SURFACE_EXTENSION_NAME);
	required_extensions.push_back(VK_KHR_WIN32_SURFACE_EXTENSION_NAME);

	// CreateSurfaceCallback is a plain function pointer, so we use a thread-local static
	// to pass the HWND to the non-capturing lambda. Safe: construction is single-threaded.
	static thread_local HWND s_surface_hwnd = nullptr;
	s_surface_hwnd = hwnd_;
	auto create_surface_callback = +[](VkInstance instance, VkSurfaceKHR* out_surface) -> bool {
		VkWin32SurfaceCreateInfoKHR info = {};
		info.sType = VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR;
		info.hinstance = GetModuleHandle(nullptr);
		info.hwnd = s_surface_hwnd;
		VkResult result = vkCreateWin32SurfaceKHR(instance, &info, nullptr, out_surface);
		return result == VK_SUCCESS;
	};

	if (!Initialize(std::move(required_extensions), create_surface_callback))
		return;
...
RenderInterface_VK::operator bool() const
{
	return initialized_;
}
...
{
	// The VK renderer clears in BeginFrame via the render pass load op (VK_ATTACHMENT_LOAD_OP_CLEAR).
}

void RenderInterface_VK::UpdateGeometryVertices(Rml::CompiledGeometryHandle /*geometry*/, Rml::Span<const Rml::Vertex> /*vertices*/)
{
	// Stub -- not yet implemented for Vulkan renderer.
}

void RenderInterface_VK::UpdateTextureData(Rml::TextureHandle /*texture_handle*/, Rml::Span<const Rml::byte> /*source_data*/, Rml::Vector2i /*source_dimensions*/)
{
	// Stub -- not yet implemented for Vulkan renderer.
}

// ---------------------------------------------------------------------------
// YUV (I420) and NV12 -> RGBA CPU conversion, rendered via the normal texture path.
// The Vulkan renderer lacks the multi-plane shader infrastructure of the DX12 renderer,
// so we convert on the CPU and feed the result through GenerateTexture / RenderGeometry.
// ---------------------------------------------------------------------------

namespace {

struct VK_YUVTextureData {
	Rml::TextureHandle rgba_texture = 0;
	uint32_t width = 0;
	uint32_t height = 0;
};

inline uint8_t clamp_byte(int v) { return static_cast<uint8_t>(v < 0 ? 0 : (v > 255 ? 255 : v)); }

// BT.601 limited-range YUV -> RGBA conversion (I420: planar Y + U + V)
void yuv_i420_to_rgba(Rml::byte* dst, const uint8_t* y, uint32_t ys,
	const uint8_t* u, const uint8_t* v, uint32_t uvs, uint32_t w, uint32_t h)
{
	for (uint32_t row = 0; row < h; ++row)
	{
		const uint8_t* yr = y + row * ys;
		const uint8_t* ur = u + (row / 2) * uvs;
		const uint8_t* vr = v + (row / 2) * uvs;
		Rml::byte* out = dst + row * w * 4;
		for (uint32_t col = 0; col < w; ++col)
		{
			int Y = yr[col];
			int U = ur[col / 2] - 128;
			int V = vr[col / 2] - 128;
			out[0] = clamp_byte(Y + ((V * 359) >> 8));           // R
			out[1] = clamp_byte(Y - ((U * 88 + V * 183) >> 8)); // G
			out[2] = clamp_byte(Y + ((U * 454) >> 8));           // B
			out[3] = 255;                                        // A
			out += 4;
		}
	}
...
// BT.601 limited-range YUV -> RGBA conversion (NV12: planar Y + interleaved UV)
void nv12_to_rgba(Rml::byte* dst, const uint8_t* y, uint32_t ys,
	const uint8_t* uv, uint32_t uvs, uint32_t w, uint32_t h)
{
	for (uint32_t row = 0; row < h; ++row)
	{
		const uint8_t* yr = y + row * ys;
		const uint8_t* uvr = uv + (row / 2) * uvs;
		Rml::byte* out = dst + row * w * 4;
		for (uint32_t col = 0; col < w; ++col)
		{
			int Y = yr[col];
			int U = uvr[(col / 2) * 2] - 128;
			int V = uvr[(col / 2) * 2 + 1] - 128;
			out[0] = clamp_byte(Y + ((V * 359) >> 8));           // R
			out[1] = clamp_byte(Y - ((U * 88 + V * 183) >> 8)); // G
			out[2] = clamp_byte(Y + ((U * 454) >> 8));           // B
			out[3] = 255;                                        // A
...
uintptr_t RenderInterface_VK::GenerateYUVTexture(
	...
	uint32_t width, uint32_t height)
{
	...
	auto* yuv = new VK_YUVTextureData();
	yuv->width = width;
	yuv->height = height;

	const size_t rgba_size = static_cast<size_t>(width) * height * 4;
	Rml::Vector<Rml::byte> rgba_buf(rgba_size);
	yuv_i420_to_rgba(rgba_buf.data(), y_data, y_stride, u_data, v_data, uv_stride, width, height);

	Rml::Span<const Rml::byte> span(rgba_buf.data(), rgba_buf.size());
	yuv->rgba_texture = GenerateTexture(span, Rml::Vector2i(static_cast<int>(width), static_cast<int>(height)));
...
void RenderInterface_VK::UpdateYUVTexture(uintptr_t handle,
	const uint8_t* y_data, uint32_t y_stride,
	const uint8_t* u_data, const uint8_t* v_data, uint32_t uv_stride,
	uint32_t width, uint32_t height)
{
	...
	auto* yuv = reinterpret_cast<VK_YUVTextureData*>(handle);

	const size_t rgba_size = static_cast<size_t>(width) * height * 4;
	Rml::Vector<Rml::byte> rgba_buf(rgba_size);
	yuv_i420_to_rgba(rgba_buf.data(), y_data, y_stride, u_data, v_data, uv_stride, width, height);

	// UpdateTextureData is a stub, so destroy and recreate.
	if (yuv->rgba_texture)
		ReleaseTexture(yuv->rgba_texture);
...
void RenderInterface_VK::ReleaseYUVTexture(uintptr_t handle)
{
	if (!handle)
		return;
...
void RenderInterface_VK::RenderYUVGeometry(Rml::CompiledGeometryHandle geometry,
	Rml::Vector2f translation, uintptr_t yuv_handle)
{
	if (!geometry || !yuv_handle)
		return;

	auto* yuv = reinterpret_cast<VK_YUVTextureData*>(yuv_handle);
	RenderGeometry(geometry, translation, yuv->rgba_texture);
}

// --- NV12 texture methods ---

uintptr_t RenderInterface_VK::GenerateNV12Texture(
	const uint8_t* y_data, uint32_t y_stride,
	...
	uint32_t width, uint32_t height)
{
	...
	auto* nv12 = new VK_YUVTextureData();
	nv12->width = width;
	nv12->height = height;

	const size_t rgba_size = static_cast<size_t>(width) * height * 4;
	Rml::Vector<Rml::byte> rgba_buf(rgba_size);
	nv12_to_rgba(rgba_buf.data(), y_data, y_stride, uv_data, uv_stride, width, height);

	Rml::Span<const Rml::byte> span(rgba_buf.data(), rgba_buf.size());
	nv12->rgba_texture = GenerateTexture(span, Rml::Vector2i(static_cast<int>(width), static_cast<int>(height)));

	if (!nv12->rgba_texture)
	{
		delete nv12;
		return 0;
	}
...
void RenderInterface_VK::UpdateNV12Texture(uintptr_t handle,
	const uint8_t* y_data, uint32_t y_stride,
	const uint8_t* uv_data, uint32_t uv_stride,
	uint32_t width, uint32_t height)
{
	...
	auto* nv12 = reinterpret_cast<VK_YUVTextureData*>(handle);
	...
	// UpdateTextureData is a stub, so destroy and recreate.
	if (nv12->rgba_texture)
		ReleaseTexture(nv12->rgba_texture);
...
void RenderInterface_VK::ReleaseNV12Texture(uintptr_t handle)
{
	if (!handle)
		return;
...
void RenderInterface_VK::RenderNV12Geometry(Rml::CompiledGeometryHandle geometry,
	Rml::Vector2f translation, uintptr_t nv12_handle)
{
	if (!geometry || !nv12_handle)
		return;

	auto* nv12 = reinterpret_cast<VK_YUVTextureData*>(nv12_handle);
	RenderGeometry(geometry, translation, nv12->rgba_texture);
}
...
void RenderInterface_VK::SetViewport(int width, int height, bool force)
{
	(void)force;
...
void RenderInterface_VK::Initialize_Swapchain(VkExtent2D window_extent) noexcept
{
	...
	info.presentMode = GetPresentMode(vsync_ ? VK_PRESENT_MODE_FIFO_KHR : VK_PRESENT_MODE_MAILBOX_KHR);
